...
public class MailSender {
    ...
    static MailResult sendTo(Addressee to, String subject, String body, List<Attachment> files) throws WebStateException {
        val state = sendToGroup(ImmutableSet.of(to), ImmutableSet.of(), subject, body, files);
        return new MailResult(to.getEmail(), state);
    }

    static String sendToGroup(Set<Addressee> to, Set<Addressee> cc, String subject, String body, List<Attachment> files) throws WebStateException {
        ...
        try {
            ...
            for (Attachment file : files) {
                email.attach(file.getDataHandler().getDataSource(), file.getName(), null);
            }
